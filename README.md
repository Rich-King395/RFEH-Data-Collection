...
- Choosing multiple recording channels for synchronized recording. Multi-channel recordings are saved independently under `Data/<experiment_folder>/<channel_id>/`, and all selected channels use the same computer-side `Time(ms)` zero point.
...
`Time(ms)` in recorded CSV files is generated by the computer and means milliseconds elapsed since recording started. The Arduino uptime sent over serial is parsed but no longer saved.

Smoothed CSV columns:

```csv
Time(ms),ADC,Voltage(V),SmoothedVoltage(V)
```
